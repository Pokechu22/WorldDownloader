...
class Utils {
	...
	public static void drawDarkBackground(int top, int left, int bottom, int right) {
		...
		wr.pos(0, bottom, 0).tex(0 / textureSize, 
				bottom / textureSize).color(32, 32, 32, 255).endVertex();
		wr.pos(right, bottom, 0).tex(right / textureSize, 
				bottom / textureSize).color(32, 32, 32, 255).endVertex();
		wr.pos(right, top, 0).tex(right / textureSize, 
				top / textureSize).color(32, 32, 32, 255).endVertex();
		wr.pos(left, top, 0).tex(left / textureSize, 
				top / textureSize).color(32, 32, 32, 255).endVertex();
		t.draw();
	}
	...
	public static void drawBorder(int topMargin, int bottomMargin, int top, int left, int bottom, int right) {
		...
		GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
		wr.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
		wr.pos(left, upperBoxEnd, 0.0D).tex(0.0D, upperBoxEnd
				/ textureSize).color(64, 64, 64, 255).endVertex();
		wr.pos(right, upperBoxEnd, 0.0D).tex(right / textureSize,
		...
		wr.pos(left, bottom, 0.0D).tex(0.0D, bottom / textureSize)
				.color(64, 64, 64, 255).endVertex();
		wr.pos(right, bottom, 0.0D).tex(right / textureSize, bottom
				/ textureSize).color(64, 64, 64, 255).endVertex();
		wr.pos(right, lowerBoxStart, 0.0D)
				.tex(right / textureSize, lowerBoxStart / textureSize)
				.color(64, 64, 64, 255).endVertex();
		wr.pos(left, lowerBoxStart, 0.0D).tex(0.0D, lowerBoxStart
				/ textureSize).color(64, 64, 64, 255).endVertex();
		t.draw();
