...
public class WDLEvents {
	...
	public static void onItemGuiOpened() {
		...
		if (WDL.minecraft.objectMouseOver.typeOfHit == MovingObjectType.ENTITY) {
			...
			WDL.lastEntity = null;
			WDL.lastClickedBlock = new BlockPos(WDL.minecraft.objectMouseOver.blockX, WDL.minecraft.objectMouseOver.blockY, WDL.minecraft.objectMouseOver.blockZ);
		}
	}
	...
	public static boolean onItemGuiClosed() {
		...
		if (WDL.thePlayer.ridingEntity != null &&
			...
			if (WDL.windowContainer instanceof ContainerHorseInventory) {
				...
				if (horseInContainer == WDL.thePlayer.ridingEntity) {
					...
					horseChest.func_110133_a(entityHorse.getCommandSenderName());
		...
		TileEntity te = WDL.worldClient.getTileEntity(WDL.lastClickedBlock.getX(), WDL.lastClickedBlock.getY(), WDL.lastClickedBlock.getZ());
		...
		//Permissions check.
		if (!WDLPluginChannels.canSaveContainers(te.xCoord << 4,
				te.zCoord << 4)) {
			WDLMessages.chatMessageTranslated(WDLMessageTypes.ON_GUI_CLOSED_INFO,
