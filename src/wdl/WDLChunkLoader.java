...
public class WDLChunkLoader extends AnvilChunkLoader {
	...
	private NBTTagCompound writeChunkToNBT(Chunk chunk, World world) {
		...
		for (ExtendedBlockStorage blockStorage : blockStorageArray) {
			if (blockStorage != null) {
				...
				blockData.setByteArray("Blocks", blockStorage.getBlockLSBArray());

				if (blockStorage.getBlockMSBArray() != null) {
					blockData.setByteArray("Add", blockStorage.getBlockMSBArray().data);
				}

				blockData.setByteArray("Data", blockStorage.getMetadataArray().data);
				blockData.setByteArray("BlockLight", blockStorage.getBlocklightArray().data);

				if (hasNoSky) {
					blockData.setByteArray("SkyLight", blockStorage.getSkylightArray().data);
				} else {
					blockData.setByteArray("SkyLight", new byte[blockStorage.getBlocklightArray().data.length]);
				}
		...
		if (updateList != null) {
			...
			for (NextTickListEntry entry : updateList) {
				...
				entryTag.setInteger("i", Block.getIdFromBlock(entry.func_151351_a()));
				entryTag.setInteger("x", entry.xCoord);
				entryTag.setInteger("y", entry.yCoord);
				entryTag.setInteger("z", entry.zCoord);
	...
	public NBTTagList getEntityList(Chunk chunk) {
		...
		for (List<?> list : chunk.entityLists) {
			// Known to be a list of entities
			List<Entity> eList = (List<Entity>)list;
	...
	public NBTTagList getTileEntityList(Chunk chunk) {
		...
		@SuppressWarnings("unchecked")
		Map<ChunkPosition, TileEntity> chunkTEMap = chunk.chunkTileEntityMap;
		...
		for (ChunkPosition pos : chunkTEMap.keySet()) {
			allTELocations.add(new BlockPos(pos.chunkPosX, pos.chunkPosY, pos.chunkPosZ));
		}
